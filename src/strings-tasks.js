 ...
 */
function removeLeadingWhitespaces(s) {
  return s.trimStart();
}
 ...
 */
function removeTrailingWhitespaces(s) {
  return s.trimEnd();
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (start > -1) {
    ...
    const str2 = str.substring(start + value.length);
...
function countVowels(str) {
  ...
  let summ = 0;
  for (let i = 0; i < strLowCase.length; i += 1) {
    if (vowels.includes(strLowCase[i])) {
      summ += 1;
    }
  }
  return summ;
...
function encodeToRot13(str) {
  ...
  const codeZ = 'a'.codePointAt(0);
  const arr = str.split('');
  const newArr = arr.map((el) => {
    if (el >= 'A' && el <= 'Z') {
      const code = el.codePointAt(0) + 13 - codeA;
      const addIndex = code % 26;
      return String.fromCodePoint(codeA + addIndex);
    }
    if (el >= 'a' && el <= 'z') {
      const code = el.codePointAt(0) + 13 - codeZ;
      const addIndex = code % 26;
      return String.fromCodePoint(codeZ + addIndex);
    }
    return el;
  });
  return newArr.join('');
